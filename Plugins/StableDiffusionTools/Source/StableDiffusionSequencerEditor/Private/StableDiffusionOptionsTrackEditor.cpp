...
bool FStableDiffusionOptionsTrackEditor::SupportsSequence(UMovieSceneSequence* InSequence) const
{
	...
	//static UClass* LevelSequencerClass = FindObject<UClass>(nullptr, TEXT("LevelSequence"), true);
	static UClass* LevelSequencerClass = FindFirstObjectSafe<UClass>(TEXT("LevelSequence"),EFindFirstObjectOptions::ExactClass);
...
void FStableDiffusionOptionsTrackEditor::OnAddTrack()
{
	...
	if (IsValid(FocusedMovieScene))
	{
		...
		UStableDiffusionOptionsTrack* NewTrack = FocusedMovieScene->AddTrack<UStableDiffusionOptionsTrack>();
