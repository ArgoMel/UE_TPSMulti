...
void UStableDiffusionMoviePipeline::SetupForPipelineImpl(UMoviePipeline* InPipeline)
{
	...
	auto Tracks = InPipeline->GetTargetSequence()->GetMovieScene()->GetTracks();
...
void UStableDiffusionMoviePipeline::RenderSample_GameThreadImpl(const FMoviePipelineRenderPassMetrics& InSampleState)
{
	...
	// Main Render Pass
	{
		...
		if (SDSubsystem) {
			...
			auto OriginalSeqFramerateRatio = TargetSequencer->GetMovieScene()->GetDisplayRate().AsDecimal() / this->GetPipeline()->GetPipelinePrimaryConfig()->GetEffectiveFrameRate(TargetSequencer).AsDecimal();
